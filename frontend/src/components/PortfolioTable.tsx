...
import { Search, ArrowUpDown, ShieldAlert, Sparkles, RefreshCw, Eye } from 'lucide-react';
...
export const PortfolioTable: React.FC<PortfolioTableProps> = ({
  ...
  // Format currency to Indian Numbering System (e.g. ₹1,50,000)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 bg-primary-light/45 p-4 rounded-xl border border-white/5 backdrop-blur-md">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search business, ID, cohort..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-primary-dark/50 border border-white/10 rounded-xl py-2 pl-9 pr-4 text-xs font-semibold text-gray-100 placeholder-gray-500 focus:outline-none focus:border-accent/40"
          />
        </div>
        ...
        <div>
          <select
            ...
            className="w-full bg-primary-dark/50 border border-white/10 rounded-xl py-2 px-3 text-xs font-semibold text-gray-300 focus:outline-none focus:border-accent/40 cursor-pointer"
          >
        ...
        <div className="flex items-center justify-end text-xs text-gray-400 font-semibold pr-1">
      ...
      <div className="border border-white/5 bg-primary-light/20 rounded-xl overflow-hidden backdrop-blur-md">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5 bg-primary-dark/40 text-gray-400 text-[10px] font-bold uppercase tracking-wider select-none">
                ...
                <th className="py-3.5 px-4">QoQ Trend</th>
                  ...
                  <tr key={item.msme_id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      <span className={`font-bold font-mono text-sm ${
                        item.percentile >= 65 ? 'text-accent-light' : (item.percentile >= 40 ? 'text-warning-light' : 'text-danger-light')
                      }`}>
                        {item.percentile}%
                      </span>
                    </td>
                    ...
                    <td className="py-3.5 px-4 font-semibold">
                      <span className={item.momentum.startsWith('-') ? 'text-danger' : 'text-accent'}>
                        {item.momentum}
                      </span>
                    </td>
                    <td className="py-3.5 px-4">
                      {item.drift_status === 'DRIFT_ALERT' ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-danger/10 border border-danger/25 text-danger text-[10px] font-bold uppercase">
                          <ShieldAlert size={12} /> Alert
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-white/5 text-gray-400 text-[10px] font-semibold">
                          Stable
                        </span>
                      )}
                    </td>
                    <td className="py-3.5 px-4 text-right">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg border border-white/5 bg-primary-light text-gray-300 hover:text-accent-light hover:bg-accent/5 transition-all disabled:opacity-40"
                          title="Rescore Business"
                        >
                          <RefreshCw size={14} className={rescoringId === item.msme_id ? 'animate-spin text-accent' : ''} />
                        </button>
                      </div>
                    </td>
                  </tr>
